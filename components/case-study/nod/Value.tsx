...
export default function Value({ title, subtitle, cards }: ValueProps) {
  return (
    <section className="w-full px-4 md:px-8 py-12">
      ...
      {/* Desktop Grid */}
      <div className="hidden max-w-[1654px] mx-auto lg:grid grid-cols-4 gap-6">
        {cards.map((card, idx) => (
          <div
            key={idx}
            className={`rounded-2xl ${cardColors[idx]} h-[403px] flex flex-col justify-center items-center p-8 text-center`}
          >
            <div className="text-xl md:text-2xl xl:text-[32px] font-semibold mb-2 text-[#232323]">{card.title}</div>
            <div className="text-5xl 2xl:text-[100px] lg:text-[72px] font-semibold text-[#fff] opacity-50 mb-2">{card.value}</div>
            <div className="text-base leading-tight tracking-tight xl:text-[24px] lg:text-[20px] text-[#232323] font-semibold">{card.subtitle}</div>
          </div>
        ))}
      </div>
      {/* Mobile/Tablet Stack */}
